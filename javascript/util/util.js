;(function (window, document, undefined) {
    ...
    // 当前tab是否在视图/焦点中
    window.BC_UTIL.isBrowserTabInView = function () {
        return document.hidden;
    };

    // 截取数字小数点后的固定几位
    window.BC_UTIL.toFixed = function (n, fixed) {
        return ~~(Math.pow(10, fixed) * n) / Math.pow(10, fixed);
    };

    // 检查元素当前是否处于焦点
    window.BC_UTIL.elementIsInFocus = function (el) {
        return el === document.activeElement;
    };

    // 是否支持touch事件
    window.BC_UTIL.touchSupported = function () {
        return ('ontouchstart' in window || window.DocumentTouch && document instanceof window.DocumentTouch);
    };

    // 当前用户是否使用Apple上
    window.BC_UTIL.isAppleDevice = function () {
        return /Mac|iPod|iPhone|iPad/.test(navigator.platform);
    };

    // 滚动到页面顶部
    window.BC_UTIL.goToTop = function () {
        return window.scrollTo(0, 0);
    };

    // 转换华氏
    window.BC_UTIL.celsiusToFahrenheit = function (celsius) {
